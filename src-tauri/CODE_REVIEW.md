...
| 优先级 | 编号 | 问题 | 状态 |
|--------|------|------|------|
| **P0** | P0-1 | `lib.rs` 2172行单文件 | **已修复** (拆分为 commands/ 模块，lib.rs 降至 240 行) |
| **P0** | P0-2 | 同步 DB 操作阻塞 async runtime | **已修复** (db.rs 部分；lib.rs command 待后续补全) |
| **P0** | P0-3 | 生产代码 `unwrap()`/`expect()` | **已修复** |
| **P1** | P1-1 | 219 次 `map_err(\|e\| e.to_string())` | **已部分修复** (基础设施已建立；command 层待迁移) |
| **P1** | P1-2 | `format!()` 拼接 SQL 表名 | **已修复** |
| **P1** | P1-3 | Database 无状态注入、无连接复用 | **已部分修复** (Database::new 返回 Result；State 注入待后续) |
| **P1** | P1-4 | XOR 加密 API Key | **待修复** |
| **P1** | P1-5 | 查询命令中内联建表语句 | **待修复** |
| **P2** | P2-1 | `env::set_var` 多线程不安全 | **已修复** |
| **P2** | P2-2 | 路径穿越未防御 | **待修复** |
| **P2** | P2-3 | spawn_blocking 样板重复 15 次 | **已修复** |
| **P2** | P2-4 | 路径规范化+计数查询重复 3 次 | **待修复** |
| **P2** | P2-5 | 窗口可见性检测代码重复 | **待修复** |

### 下一步建议

1. **P1-3 剩余（Database State 注入）** — 在 `setup` 中 `app.manage(db)` 并修改所有 command 签名接收 `State<'_, Database>`，消除重复的 `Database::new()`
2. **P1-1 剩余（command 层迁移 AppResult）** — 将 command 函数的返回值从 `Result<T, String>` 迁移到 `AppResult<T>`，消除 `.map_err(|e| e.to_string())`
3. **P0-2 剩余（command 层 spawn_blocking）** — 将 commands/ 中的同步 DB 操作包裹到 `spawn_blocking`
4. **P1-5（移除内联建表）** — 小改动，随时可做
5. **P2-2（路径穿越防御）** — 安全相关，建议尽早处理
6. **P1-4（API Key 加密）** — 需引入新依赖，可独立排期
7. **P2-4 / P2-5（DRY 重复）** — 提取公共函数消除重复
